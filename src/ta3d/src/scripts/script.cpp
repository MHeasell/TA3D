...
namespace TA3D
{
	...
	void generate_script_from_mission(String Filename, TDFParser& ota_parser, int schema)
	{
		...
		if (!ota_parser.pullAsString("GlobalHeader.AnyUnitPassesZ").empty() || !ota_parser.pullAsString("GlobalHeader.UnitTypePassesZ").empty() || !ota_parser.pullAsString("GlobalHeader.AnyUnitPassesX").empty() || !ota_parser.pullAsString("GlobalHeader.UnitTypePassesX").empty())
		{
