...
TA3D::AudioManager* TA3D::sound_manager;
...
#warning TODO: implement 3D stereo
	...
	void AudioManager::playSound(const String& filename, const Vector3D* vec)
	{
		...
		MutexLocker locker(pMutex);
		if (vec && Camera::inGame && ((Vector3D) (*vec - Camera::inGame->position())).lengthSquared() > 360000.0f) // If the source is too far, does not even think about playing it!
			return;
		if (!m_SDLMixerRunning)
			return;
