		...
		do
		{
			...
			if (selected && (!IsOnGUI || IsOnMinimap))
			{
				...
				if (pointing >= 0 && !rope_selection) // S'il y a quelque chose sous le curseur
				{
					...
					if (cursor_type == CURSOR_MOVE && canguard && pointing != cur_sel_index && units.unit[pointing].owner_id == players.local_human_id)
						cursor_type = CURSOR_GUARD;
						...
						else
						{
							if (cursor_type == CURSOR_REPAIR)
							{
								for (unsigned int e = 0; e < units.index_list_size; ++e)
								{
									...
									units.unlock();
									units.unit[i].lock();
									if ((units.unit[i].flags & 1) && units.unit[i].owner_id == players.local_human_id && units.unit[i].sel && unit_manager.unit_type[units.unit[i].type_id]->Builder && unit_manager.unit_type[units.unit[i].type_id]->BMcode)
									{
										if (!TA3D_SHIFT_PRESSED)
							...
							else
							{
								if (cursor_type == CURSOR_RECLAIM)
								{
									for (unsigned int e = 0; e < units.index_list_size; ++e)
									{
										...
										units.unlock();
										units.unit[i].lock();
										if ((units.unit[i].flags & 1) && units.unit[i].owner_id == players.local_human_id && units.unit[i].sel && unit_manager.unit_type[units.unit[i].type_id]->CanReclamate && unit_manager.unit_type[units.unit[i].type_id]->BMcode)
										{
											if (TA3D_SHIFT_PRESSED)
						...
						if (left_click_activation)
						{
							if (cursor_type == CURSOR_ATTACK || cursor_type == CURSOR_BOMB_ATTACK)
							{
								...
								for (unsigned int e = 0; e < units.index_list_size; ++e)
								{
									...
									int i = units.idx_list[e];
									units.unit[i].lock();
									if ((units.unit[i].flags & 1) && units.unit[i].owner_id == players.local_human_id && units.unit[i].sel && unit_manager.unit_type[units.unit[i].type_id]->canattack && (unit_manager.unit_type[units.unit[i].type_id]->BMcode || !unit_manager.unit_type[units.unit[i].type_id]->Builder))
									{
										for (unsigned int f = 0; f < unit_manager.unit_type[units.unit[i].type_id]->weapon.size(); ++f)
											if (unit_manager.unit_type[units.unit[i].type_id]->weapon[f] && unit_manager.unit_type[units.unit[i].type_id]->weapon[f]->stockpile)
											{
			...
			if (cursor_type == CURSOR_REVIVE && CURSOR_REVIVE != CURSOR_RECLAIM && !rope_selection && click_activation && (!IsOnGUI || IsOnMinimap) && !order_removed) // The cursor orders to resurrect a wreckage
			{
				...
				if (idx >= 0 && features.feature[idx].type >= 0 && feature_manager.getFeaturePointer(features.feature[idx].type)->reclaimable)
				{
					for (unsigned int e = 0; e < units.index_list_size; ++e)
					{
						...
						units.unlock();
						units.unit[i].lock();
						if ((units.unit[i].flags & 1) && units.unit[i].owner_id == players.local_human_id && units.unit[i].sel && unit_manager.unit_type[units.unit[i].type_id]->canresurrect && unit_manager.unit_type[units.unit[i].type_id]->BMcode)
						{
							if (TA3D_SHIFT_PRESSED)
			...
			if (TA3D_CTRL_PRESSED && (isKeyDown(KEY_C) || isKeyDown(KEY_F) || isKeyDown(KEY_V) || isKeyDown(KEY_B)))
			{
				...
				{
					int i = units.idx_list[e];
					if ((units.unit[i].flags & 1) && units.unit[i].owner_id == players.local_human_id && Math::AlmostZero(units.unit[i].build_percent_left))
					{
			...
			else if (TA3D_CTRL_PRESSED && isKeyDown(KEY_Z)) // Sélectionne toutes les unités dont le type est déjà sélectionné / Select units of the same type
			{
				...
				{
					const size_t i = units.idx_list[e];
					if ((units.unit[i].flags & 1) && Math::AlmostZero(units.unit[i].build_percent_left) && units.unit[i].owner_id == players.local_human_id && sel_type[units.unit[i].type_id])
						units.unit[i].sel = true;
				}
				cur_sel = -1;
				cur_sel_index = -1;
			...
			else
			{
				if (TA3D_CTRL_PRESSED && isKeyDown(KEY_A)) // Select all the player's units
				{
					...
					{
						int i = units.idx_list[e];
						if ((units.unit[i].flags & 1) && Math::AlmostZero(units.unit[i].build_percent_left) && units.unit[i].owner_id == players.local_human_id)
							units.unit[i].sel = true;
					}
					cur_sel = -1;
					cur_sel_index = -1;
