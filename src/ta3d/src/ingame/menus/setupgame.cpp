	...
	namespace Menus
	{
		...
		bool SetupGame::maySwitchToAnotherMenu()
		{
			...
			for (int i = 0; i < TA3D_PLAYERS_HARD_LIMIT; ++i)
			{
				...
				if (saved_game.notEmpty())
					continue; // We mustn't change any thing for a saved game
				...
				{
					if (((!client && !(game_data.player_control[i] & PLAYER_CONTROL_FLAG_REMOTE)) || (client && game_data.player_control[i] == PLAYER_CONTROL_LOCAL_HUMAN)) && game_data.player_control[i] != PLAYER_CONTROL_NONE && game_data.player_control[i] != PLAYER_CONTROL_CLOSED)
					{
						network_manager.sendSpecial("NOTIFY UPDATE");
				...
				if (pArea->get_state(String("gamesetup.b_energy") << i)) // Change player energy stock
				{
					...
					if (game_data.energy[i] == 0)
						game_data.energy[i] = 500;
				...
				if (pArea->get_state(String("gamesetup.b_metal") << i)) // Change player metal stock
				{
					...
					if (game_data.metal[i] == 0)
						game_data.metal[i] = 500;
			...
			if (minimap_obj != NULL && (((pArea->get_state("gamesetup.minimap") || pArea->get_state("gamesetup.change_map")) && !client) || (client && !set_map.empty())) && saved_game.empty()) // Clic on the mini-map or received map set command
			{
				...
				if (!new_map.empty() && (set_map.empty() || (client && VFS::Instance()->fileExists(Paths::Files::ReplaceExtension(new_map, ".tnt")) && VFS::Instance()->fileExists(Paths::Files::ReplaceExtension(new_map, ".ota")))))
				{
					set_map.clear();
					if (host.notEmpty() && !client)
					{
					...
					case 2:
						if (params[0] == "REQUEST")
						{
							...
							else if (params[1] == "GameData") // Sending game information
							{
								...
								{
									if (client && game_data.player_network_id[i] != my_player_id)
										continue; // We don't send updates about things we won't update
