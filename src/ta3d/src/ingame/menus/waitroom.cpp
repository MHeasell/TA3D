...
namespace TA3D
{
	namespace Menus
	{
		...
		bool WaitRoom::doInitialize()
		{
			...
			{
				dead_player[i] = true;
				pArea->msg(String("wait.name") << i << ".hide");
				pArea->msg(String("wait.progress") << i << ".hide");
				pArea->msg(String("wait.ready") << i << ".hide");
			}
			...
			for (int i = 0; i < game_data->nb_players; ++i)
			{
				...
				{
					pArea->set_data(String("wait.progress") << i, 100);
					pArea->set_state(String("wait.ready") << i, true);
				}
				else
					pArea->set_state(String("wait.ready") << i, false);
				pArea->caption(String("wait.name") << i, game_data->player_names[i]);
			}
		...
		void WaitRoom::waitForEvent()
		{
			...
			} while (pMouseX == mouse_x && pMouseY == mouse_y && pMouseZ == mouse_z && pMouseB == mouse_b && mouse_b == 0 && !key[KEY_ENTER] && !key[KEY_ESC] && !key[KEY_SPACE] && !key[KEY_C] && !pArea->key_pressed && !pArea->scrolling && special_msg.empty() && !playerDropped && (msec_timer - ping_timer < 2000 || !network_manager.isServer()));
		}
		...
		bool WaitRoom::maySwitchToAnotherMenu()
		{
			...
			if (network_manager.isServer() && check_ready) // If server is late the game should begin once he is there
			{
				...
				{
					if (!pArea->get_state(String("wait.ready") << i) && !dead_player[i] && game_data->player_network_id[i] > 0)
						ready = false;
				}
